...
| 22 | Design system Lumiz NB Clinic + admin global RBAC (`GET /api/user/whoami`) | Back + Front | G | ✅ Concluído (back + front) — 09/05/2026 — Backend: `GET /api/user/whoami` (commit `6e9cdf4`) + 6 testes. Front: app migrado para o design system de `lumiz-nb-clinic.html`; 5 páginas admin (`/admin`, `/admin/usuarios`, `/admin/assinaturas`, `/admin/feedback`, `/admin/diagnostico`) com gating via `whoami`; `npx tsc --noEmit` e `npm run build` verdes. 🟡 Follow-up de produto: limpar blocos extras do sidebar (Operações extras, outlook, pricing) fora das 17 abas do espec — PR separada. Detalhes: `lumiz-financeiro/HANDOFF_FRONTEND.md`. |
