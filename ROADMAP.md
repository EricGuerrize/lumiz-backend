...
| 15 | Audit log | Back + Front | M | ✅ Back concluído (08/05/2026) — front Audit Log UI pushado em 09/05/2026: branch `feat/audit-log-fase15`, commit `530c206` → `origin/feat/audit-log-fase15`; mesmo PR/consolidado em **`79c9a4a`**: cleanup definitivo do sidebar + hooks LGPD (`useConsentStatus` / `useAcceptCurrentTerms`); compare p/ PR: https://github.com/EricGuerrize/lumiz-financeiro/compare/feat/audit-log-fase15 |
...
**Frontend concluído:** ContasPagar (+ pró-labore), Pró-labore (`/dashboard/prolabore`), Colaboradores (`/dashboard/colaboradores`), Cashflow, Simulador (multi-período), Precificação + Custo Real (comissão no breakdown), Estoque, NF/Validade, Inadimplência, Sazonalidade, Health Score (`HealthScoreCard` + opcional `cobertura_fornecedor`), Goals/Meta (3 campos), Emergency (detalhes + histórico), Export (PDF/CSV), Sidebar (estrutura final 20 itens em 4 grupos — ver `implementacao2.md` cleanup 09/05/2026). **Onda 2–4:** Documentos de fornecedor (`/dashboard/supplier-docs`), Fornecedores, Contas a receber, painéis Alter (`/dashboard/alter/*`). **Tabela #22 (09/05/2026 — back + front):** backend entregou `GET /api/user/whoami` (commit `6e9cdf4`, 6 testes; regressão 216/216); front migrou o `apps/dashboard` ao design system de `lumiz-nb-clinic.html` e liberou páginas admin com RBAC via `whoami`. **✅ Follow-up de produto (sidebar)** concluído em 09/05/2026 — commit **`79c9a4a`**, branch `feat/audit-log-fase15`, compare p/ PR: https://github.com/EricGuerrize/lumiz-financeiro/compare/feat/audit-log-fase15; verificações `tsc --noEmit` e `npm run build` OK no front.
